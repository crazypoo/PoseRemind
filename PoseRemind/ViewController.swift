...
class ViewController: UIViewController {
    ...
    var threadStepperLabel: UILabel = {
        let view = UILabel()
        return view
    }()
    var threadStepper: UIStepper = {
        let view = UIStepper()
        return view
    }()
    ...
    lazy var delegatesSegmentedControl: UISegmentedControl = {
        let view = UISegmentedControl()
        view.addTarget(self, action: #selector(self.delegatesValueChanged(_:)), for: .valueChanged)
        return view
    }()
    var modelSegmentedControl: UISegmentedControl = {
        let view = UISegmentedControl()
        return view
    }()
    ...
    private func someViewSet()
    {
        self.view.addSubviews([self.overlayView,self.delegatesSegmentedControl,self.modelSegmentedControl,self.scoreLabel,self.totalTimeLabel])
        self.overlayView.snp.makeConstraints { make in
            make.left.right.equalToSuperview()
            make.top.equalToSuperview().inset(kNavBarHeight_Total)
            make.bottom.equalToSuperview().inset(kTabbarHeight_Total + 100)
        }
        self.scoreLabel.snp.makeConstraints { make in
            make.left.right.equalToSuperview().inset(10)
            make.top.equalTo(self.overlayView.snp.bottom).offset(10)
        }
        self.totalTimeLabel.snp.makeConstraints { make in
            make.left.right.equalTo(self.scoreLabel)
            make.top.equalTo(self.scoreLabel.snp.bottom)
        }
        self.delegatesSegmentedControl.snp.makeConstraints { make in
            make.left.right.equalTo(self.scoreLabel)
            make.top.equalTo(self.totalTimeLabel.snp.bottom)
    ...
    private func threadStepperValueChanged(_ sender: UIStepper) {
      threadCount = Int(sender.value)
      threadStepperLabel.text = "\(threadCount)"
      updateModel()
    }
    @objc private func delegatesValueChanged(_ sender: UISegmentedControl) {
      delegate = Delegates.allCases[sender.selectedSegmentIndex]
      updateModel()
    }
    private func modelTypeValueChanged(_ sender: UISegmentedControl) {
